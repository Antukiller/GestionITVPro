...
public class VehiculoJsonRepository : IVehiculoRepository {
    ...
    private readonly Dictionary<int, VehiculoEntity> _porId = new();
    ...
    public Result<Vehiculo, DomainError> Update(int id, Vehiculo model) {
        if (!_porId.TryGetValue(id, out var actual)) return Result.Failure<Vehiculo, DomainError>(VehiculoErrors.NotFound(id.ToString()));
        ...
        if (actual.DniPropietario != nuevoDni) {
            if (_dniPropietario.TryGetValue(nuevoDni, out var listaDestino) && listaDestino.Count >= 3) {
                return Result.Failure<Vehiculo, DomainError>(VehiculoErrors.Validation(["Límite de 3 vehículos alcanzado"]));
            }
        }
    ...
    public Result<Vehiculo, DomainError> Restore(int id) {
        ...
        var restored = new VehiculoEntity {
            Id = entity.Id,
        ...
        return Result.Success<Vehiculo, DomainError>(restored.ToModel()!);
    }
    ...
    private void Load() {
        try {
            ...
            var entities = JsonSerializer.Deserialize<List<VehiculoEntity>>(json, _jsonOptions);
