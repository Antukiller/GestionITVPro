...
public class VehiculoDapperRepository : IVehiculoRepository {
    ...
    public Result<Vehiculo, DomainError> Create(Vehiculo model) {
        ...
        try {
            var sql =
                @"INSERT INTO Vehiculos (Matricula, Marca, Modelo, Cilindrada, Motor, DniPropietario, CreatedAt, UpdatedAt, IsDeleted, DeletedAt)
                VALUES (@Matricula, @Marca, @Modelo, @Cilindrada, @Motor, @DniPropietario, @CreatedAt, @UpdatedAt, @IsDeleted, @DeletedAt);
        ...
        try {
            var sql =
                @"UPDATE Vehiculos SET
                  ...
                  CreatedAt = @CreatedAt, UpdatedAt = @UpdatedAt, IsDeleted = @IsDeleted, DeletedAt = @DeletedAt
                WHERE Id = @Id";
            ...
            var a = GetById(id);
            return a != null
                ? Result.Success<Vehiculo, DomainError>(a)
                : Result.Failure<Vehiculo, DomainError>(VehiculoErrors.DatabaseError("Error al recuperar tras actualizar"));
    ...
    public Vehiculo? GetByDniPropietario(string dniPropietario) {
        ...
        return _connection.QueryFirstOrDefault<VehiculoEntity>(sql, new { DniPropietario = dniPropietario })?.ToModel();
    }
